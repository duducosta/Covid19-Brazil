setwd(file.path("D:","Projects","Data Analysis","Covid19 - Brazil",last_update))
getwd()
...
for(var in unique(df_estados$uf))
{
  ...
  p1 <- ggplot(data=df_estados[df_estados$uf == var,]) + 
    ...
    #geom_bar(aes(x=datetime, y=max(df_estados$cases),alpha=0.5))+
    labs(title = paste("Casos totais no estado de",var, "por dia" ),
         subtitle = "",
         caption = paste("Origem dos dados:",origem,"\n Atualizado em:", last_update,"   -  Autor: Eduardo Costa"),
  ...
  jpeg(paste(var,"-ColunasComVel.jpeg"), width=1000, height=800)
  print(p1)
  dev.off()
  df_estados$TanAngle <-NULL
...
p1 <- ggplot(data=p1_data) + 
  ...
  theme(axis.title = element_text(face="bold",colour="red",size=20),
        ...
        plot.title = element_text(lineheight = 0.8,face="bold",size=30),
        plot.caption = element_text(lineheight = 0.8,size=15))
jpeg(paste(var,"Brasil-SemFiltro-Cartesiana.jpeg"), width=1000, height=800)
print(p1)
dev.off() 
remove(p1)
...
p3 <- ggplot(data=p3_data) + 
  ...
  theme(axis.title = element_text(face="bold",colour="red",size=20),
        ...
        plot.title = element_text(lineheight = 0.8,face="bold",size=30),
        plot.caption = element_text(lineheight = 0.8,size=15))
jpeg(paste(var,"Brasil-ComFiltro-Cartesiana.jpeg"), width=1000, height=800)
print(p3)
dev.off() 
remove(p3)
...
p3 <- ggplot(data=p3_data) + 
  ...
  theme(axis.title = element_text(face="bold",colour="red",size=20),
        ...
        plot.title = element_text(lineheight = 0.8,face="bold",size=30),
        plot.caption = element_text(lineheight = 0.8,size=15))
jpeg(paste(var,"Brasil-ComFiltro-Logaritma.jpeg"), width=1000, height=800)
print(p3)
dev.off() 
...
dev.off() 
remove(p2)
jpeg("Brasil-TreemapAtual-Tab1.jpeg", width=1000, height=800)
grid.newpage()
grid.table(p2_data[1:10,])
dev.off() 
jpeg("Brasil-TreemapAtual-Tab2.jpeg", width=1000, height=800)
grid.newpage()
grid.table(p2_data[11:20,])
dev.off() 
jpeg("Brasil-TreemapAtual-Tab3.jpeg", width=1000, height=800)
grid.newpage()
